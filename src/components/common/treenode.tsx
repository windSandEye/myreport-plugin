...
export const drawNode = (ctx, node, x, y, width = 300, height = 150,
    ...
    drawRingPie(ctx, node, x + width * 0.25, y, radius, innerRadius, scale,
        pieColor, mousePoint, isRingRange, treePage);
}
 ...
 * innerRadius:内层圆半径
 */
export const drawRingPie = (ctx, node, x, y, radius, innerRadius, scale,
    color, mousePoint, isRingRange, treePage) => {
    radius = radius * scale;
    ...
    for (let i = 0; i < node.pieData.length; i++) {
        ...
        if (mousePoint && ctx.isPointInPath(mousePoint.x, mousePoint.y) && isRingRange) {//鼠标点击了并且在该部分圆环上
            ...
            drawDynamicPie(ctx, node, x, y, radius, color, i);//重绘圆
            let tipNode = {
                desc: node.desc,
...
//绘制动态圆
const drawDynamicPie = (ctx, node, x, y, radius, color, index) => {
    let startRadian = 0, endRadian = 0;
    for (let i = 0; i < node.pieData.length; i++) {
        ctx.beginPath();
        //起始点移动到圆心
        ...
        //以圆心为起点，0度开始绘制一个圆
        if (index == i) {
            ctx.arc(x, y, radius + 5, startRadian, endRadian, false);
        } else {
            ctx.arc(x, y, radius, startRadian, endRadian, false);
        }
        ctx.closePath();
...
//点击位置是否在圆环上（数据为列表）
export const isRingPostion = (mousePoint, treeData, nodeWidth, innerRadius, radius, scale) => {
    if (!mousePoint) {
        return false;
    }
